...
line_list_filename = "G9.espresso.mas"  # or K2?
 ...
 line_list_espresso = prepare_line_list(linelist_for_ccf_fn_w_path, all_spectra, pipeline_plan, v_center_to_avoid_tellurics=ccf_mid_velocity,
      Δv_to_avoid_tellurics = 2*RvSpectMLBase.max_bc+3*lsf_width,#= orders_to_use=lfc_orders, =# recalc=true, verbose=true)
...
msf = 4.0; fwtf = 2.0
 ...
 ((ccfs_espresso, ccf_vars_espresso), v_grid) = ccf_total(order_list_timeseries, line_list_espresso, pipeline_plan,  mask_scale_factor=msf, range_no_mask_change=0*line_width_05+5*line_width_50, ccf_mid_velocity=ccf_mid_velocity, v_step=100,
...
for (i,ord) in enumerate(max_orders)
   ...
   if sum(view(order_ccfs,:,i,1)) > 0
      ...
      alg_fit_rv3 =  EchelleCCFs.MeasureRvFromCCFTemplate(v_grid=v_grid, frac_of_width_to_fit=fwtf, template = vec(mean(view(order_ccfs,:,i,:),dims=2)) )
