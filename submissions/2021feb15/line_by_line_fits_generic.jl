...
#all_spectra = include(joinpath(pkgdir(EchelleInstruments),"examples/read_expres_data_34411.jl"))
...
#order_list_timeseries = extract_orders(all_spectra,pipeline_plan, orders_to_use=43:72, recalc=true )
#order_list_timeseries = extract_orders(all_spectra,pipeline_plan, orders_to_use=53:62, recalc=true )
...
if make_plot(pipeline_plan,:template)
   ...
   plot!(plt,order_list_timeseries[1][chunkid].λ,order_list_timeseries[1][chunkid].flux, markersize=1.2,label="Obs  1")
   plot!(plt,order_list_timeseries[10][chunkid].λ,order_list_timeseries[10][chunkid].flux, markersize=1.2,label="Obs 10")
   plot!(plt,order_list_timeseries[20][chunkid].λ,order_list_timeseries[20][chunkid].flux, markersize=1.2,label="Obs 20")
...
need_to!(pipeline_plan,:fit_lines)
 if need_to(pipeline_plan,:fit_lines)
   ...
   @time fits_to_lines = RvSpectML.LineFinder.fit_all_lines_in_chunklist_timeseries(order_list_timeseries, lines_in_template, plan=line_finder_plan, rvs=rvs_ccf )
...
function select_line_fits_with_good_std_v(line_fits_df::DataFrame, quantile_threshold::Real; width_quantile_threshold::Real = 0.98, verbose::Bool = false, write_csv::Bool = false)
   ...
   fit_distrib = line_fits_df |> @filter(_.fit_converged == 1) |> @groupby(_.line_id) |>
            ...
            @join(conv_frac, _.line_id, _.line_id, {line_id=_.line_id, median_a=_.median_a, median_b=_.median_b, median_depth=_.median_depth, median_σ²=_.median_σ², median_λc=_.median_λc,
                         std_a=_.std_a, std_b=_.std_b, std_depth=_.std_depth, std_σ²=_.std_σ², std_λc=_.std_λc, frac_converged=__.frac_converged } ) |>
...
function calc_rvs_from_line_by_line_fits_stdv(fits_to_lines::DataFrame, threshold_stdv::Real)
   ...
   df = fits_to_lines |> @join(good_lines_stdv, _.line_id, _.line_id, {obs_id=_.obs_idx, line_id=_.line_id, Δv=(_.fit_λc.-__.median_λc).*RvSpectML.speed_of_light_mps./__.median_λc, weight=weight=(min(__.median_λc/__.std_λc, RvSpectML.speed_of_light_mps/750))^2 }) |>
...
good_lines_alt = select_line_fits_with_good_depth_width_slope(fits_to_lines,0.8)
df2 = fits_to_lines |> @join(good_lines_alt, _.line_id, _.line_id, {obs_id=_.obs_idx, line_id=_.line_id, Δv=(_.fit_λc.-__.median_λc).*RvSpectML.speed_of_light_mps./__.median_λc, weight=(min(__.median_λc/__.std_λc, RvSpectML.speed_of_light_mps/750))^2 }) |>
         @groupby(_.obs_id) |> @map({obs_id=first(_.obs_id), Δv= sum(_.Δv.*_.weight)./sum(_.weight) }) |> DataFrame
...
lftmp = select_line_fits_with_good_depth_width_slope(fits_to_lines,0.8)

scatter(thresholds,map(t->size(select_line_fits_with_good_depth_width_slope(fits_to_lines,t),1),thresholds), label="alt")
 xlabel!("Threshold for accepting lines")
 ylabel!("Number of lines")

scatter(thresholds,map(t->sum(select_line_fits_with_good_depth_width_slope(fits_to_lines,t).median_depth),thresholds), label="alt")
  scatter!(thresholds,map(t->sum(select_line_fits_with_good_std_v(fits_to_lines,t).median_depth),thresholds), label="std_v")
  xlabel!("Threshold for accepting lines")
  ylabel!("Sum of line depths")


fit_tmp = fits_to_lines |> @groupby(_.line_id) |>
                    @map( { median_a=median(_.fit_a), median_b=median(_.fit_b), median_depth=median(_.fit_depth), median_σ²=median(_.fit_σ²), median_λc=median(_.fit_λc),
                           std_a=std(_.fit_a), std_b=std(_.fit_b), std_depth=std(_.fit_depth), std_σ²=std(_.fit_σ²), std_λc=std(_.fit_λc),
                           line_id=first(_.line_id),  frac_converged=mean(_.fit_converged)  } )  |>
                    @filter(_.median_depth > 0.05 ) |> @filter(_.frac_converged > 0.5 ) |> DataFrame
         histogram(fit_tmp.frac_converged)

size(good_lines_alt) ,size(good_lines_stdv), size(fit_tmp)

using CSV
map(t->select_line_fits_with_good_depth_width_slope(fits_to_lines,t, write_csv=true),thresholds)
...
CSV.write("line_list.tmp", good_lines_alt |> @map( {lambda=_.median_λc, depth=_.median_depth} ) |> DataFrame )
...
((ccfs3, ccf_vars3), v_grid3) = ccf_total(order_list_timeseries, line_list_df_tmp, pipeline_plan,  mask_scale_factor=8.0, range_no_mask_change=line_width_05, ccf_mid_velocity=ccf_mid_velocity, v_step=100, calc_ccf_var=true, recalc=true)
rvs_ccf = RvSpectML.calc_rvs_from_ccf_total(ccfs3, ccf_vars3, pipeline_plan, v_grid=v_grid3, times = order_list_timeseries.times, recalc=true)
