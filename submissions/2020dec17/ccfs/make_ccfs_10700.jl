...
verbose = true
 ...
 if need_to(pipeline_plan,:read_spectra)
    ...
    @time all_spectra = map(row->EXPRES.read_data(row,store_min_data=true, store_tellurics=true, store_blaze=true, store_continuum=true, store_pixel_mask=true,
         normalization=:continuum),eachrow(df_files_use))
    #@time all_spectra = map(row->EXPRES.read_data(row,store_min_data=true, store_tellurics=true, store_blaze=true, store_continuum=true, store_pixel_mask=true, normalization=:blaze),eachrow(df_files_use))
    GC.gc()
...
linelist_for_ccf_fn_w_path = joinpath(pkgdir(EchelleCCFs),"data","masks","G8.espresso.mas")
 line_list_espresso = prepare_line_list(linelist_for_ccf_fn_w_path, all_spectra, pipeline_plan, v_center_to_avoid_tellurics=ccf_mid_velocity, Δv_to_avoid_tellurics = RvSpectMLBase.max_bc,
   orders_to_use=lfc_orders, recalc=true, verbose=true)
 #CSV.write(joinpath("pennstate","10700","10700_line_list_espresso.csv"), line_list_espresso)
...
title!("10700: CCF - <CCF>, ESPRESSO mask + SNR weights")
...
msf = 4.0; fwtf = 1.5
 ...
 #fwtf = 0.4; println("# mask_scale_factor = ", msf, ", frac_width_to_fit =  ", fwtf, ". ")
...
#xlims!(ccf_mid_velocity-3*line_width_50,ccf_mid_velocity+3*line_width_50)
savefig("ccf_heatmaps_10700_espresso_msf=4.0.png")
