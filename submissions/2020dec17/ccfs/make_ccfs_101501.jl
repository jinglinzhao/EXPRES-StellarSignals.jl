...
linelist_for_ccf_fn_w_path = joinpath(pkgdir(EchelleCCFs),"data","masks","G9.espresso.mas")  # or K2?
 ...
 line_list_espresso = prepare_line_list(linelist_for_ccf_fn_w_path, all_spectra, pipeline_plan, v_center_to_avoid_tellurics=ccf_mid_velocity, Δv_to_avoid_tellurics = RvSpectMLBase.max_bc,#= orders_to_use=lfc_orders, =# recalc=true, verbose=true)
...
#heatmap(v_grid,1:size(ccfs_norm,2),ccfs_norm')
heatmap(v_grid,1:size(ccfs_norm,2),(ccfs_norm.-ccf_template)')
xlabel!("v (m/s)")
ylabel!("Observation ID")
title!("101501: CCF - <CCF>, ESPRESSO mask + SNR weights")
savefig("ccf_heatmaps_101501.png")
...
line_list_espresso_old = copy(line_list_espresso)
 line_list_espresso = prepare_line_list(linelist_for_ccf_fn_w_path, all_spectra, pipeline_plan, v_center_to_avoid_tellurics=ccf_mid_velocity, Δv_to_avoid_tellurics = RvSpectMLBase.max_bc+2.5*line_width_50,  orders_to_use=lfc_orders, recalc=true, verbose=true)
 CSV.write(joinpath("pennstate","101501","101501_line_list_espresso.csv"), line_list_espresso)
...
#xlims!(ccf_mid_velocity-3*line_width_50,ccf_mid_velocity+3*line_width_50)
savefig("ccf_heatmaps_101501_espresso_msf=4.0.png")
...
xlabel!("v (m/s)")
ylabel!("Order index")
title!("101501: CCF_" *string(obs_id) * ", ESPRESSO mask + SNR weights")
#xlims!(ccf_mid_velocity-RvSpectMLBase.max_bc,ccf_mid_velocity+RvSpectMLBase.max_bc)
xlims!(ccf_mid_velocity-3*line_width_50,ccf_mid_velocity+3*line_width_50)
