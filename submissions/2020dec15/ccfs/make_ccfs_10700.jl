...
((ccfs_espresso, ccf_vars_espresso), v_grid) = ccf_total(order_list_timeseries, line_list_espresso, pipeline_plan,  mask_scale_factor=1.4, range_no_mask_change=0*line_width_05+3*line_width_50, ccf_mid_velocity=ccf_mid_velocity, v_step=100,
   #v_max=RvSpectMLBase.max_bc,
   v_max= 2*RvSpectMLBase.max_bc, #+2*line_width_50,
   calc_ccf_var=true, recalc=true)
alg_fit_rv =  EchelleCCFs.MeasureRvFromCCFGaussian(frac_of_width_to_fit=0.4)
...
xlims!(ccf_mid_velocity-3*line_width_50,ccf_mid_velocity+3*line_width_50)
savefig("ccf_heatmaps_10700_espresso_msf=1.4.png")
